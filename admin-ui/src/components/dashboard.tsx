...
export function Dashboard({ onLogout }: DashboardProps) {
    ...
    () => {
      ...
      try {
        ...
        if (cached) {
          ...
          return new Map(
            Object.entries(parsed).map(([id, balance]) => [
              Number(id),
  ...
  const disabledCredentialCount =
    data?.credentials.filter((credential) => credential.disabled).length || 0;
  ...
  // 计算全部凭据总剩余量
  const totalRemaining = Array.from(balanceMap.values()).reduce(
    (sum, b) => sum + b.remaining,
    0,
  );
  const totalLimit = Array.from(balanceMap.values()).reduce(
    (sum, b) => sum + b.usageLimit,
    0,
  );
  ...
  useEffect(() => {
    ...
    const validIds = new Set(
      data.credentials.map((credential) => credential.id),
    );
  ...
  useEffect(() => {
    ...
    const idsToFetch = data.credentials
      .filter(
        (c) =>
          !c.disabled && !balanceMap.has(c.id) && !loadingBalanceIds.has(c.id),
      )
      .map((c) => c.id);
  ...
  const handleBatchDelete = async () => {
    ...
    const skippedText =
      skippedCount > 0 ? `（将跳过 ${skippedCount} 个未禁用凭据）` : "";
    ...
    const skippedResultText =
      skippedCount > 0 ? `，已跳过 ${skippedCount} 个未禁用凭据` : "";

    if (failCount === 0) {
      toast.success(
        `成功删除 ${successCount} 个已禁用凭据${skippedResultText}`,
      );
    } else {
      toast.warning(
        `删除已禁用凭据：成功 ${successCount} 个，失败 ${failCount} 个${skippedResultText}`,
      );
    }
  ...
  const handleClearAll = async () => {
    ...
    const disabledCredentials = data.credentials.filter(
    ...
    if (
      !confirm(
        `确定要清除所有 ${disabledCredentials.length} 个已禁用凭据吗？此操作无法撤销。`,
      )
    ) {
      return;
    }
    ...
    if (failCount === 0) {
      ...
      toast.warning(
        `清除已禁用凭据：成功 ${successCount} 个，失败 ${failCount} 个`,
      );
    }
  ...
  const handleQueryCurrentPageInfo = async () => {
    ...
    const ids = currentCredentials
      .filter(
        (credential) => !credential.disabled && !balanceMap.has(credential.id),
      )
  ...
  return (
    <div className="min-h-screen bg-background">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {data?.credentials && data.credentials.length > 0 && (
                    ...
                    >
                      <RefreshCw
                        className={`h-3 w-3 mr-1 ${queryingInfo ? "animate-spin" : ""}`}
                      />
                      {queryingInfo
                  ...
                  >
                    ...
                    <span className="hidden sm:inline">KAM</span>导入
                  </Button>
                  <Button
                ...
                <>
                  <div className="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-4 -mx-4 px-4 md:mx-0 md:px-0 md:grid md:grid-cols-2 lg:grid-cols-3 md:overflow-x-visible md:snap-none md:pb-0">
                    {currentCredentials.map((credential) => (
                      <div
                        key={credential.id}
                        className="min-w-[calc(100vw-2rem)] snap-center md:min-w-0"
                      >
                        <CredentialCard
      ...
      <div className="text-center text-xs text-muted-foreground py-4">
        Kiro Admin v{__APP_VERSION__} · Build:{" "}
